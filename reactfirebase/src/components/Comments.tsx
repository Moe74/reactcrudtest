...
function Comments() {
    ...
    const { isLoggedIn, isAdmin, name: loggedInName, email: loggedInEmail } = useAuth();
    ...
    React.useEffect(() => {
        ...
        onValue(commentsRef, (snapshot) => {
            ...
            snapshot.forEach(childSnapshot => {
                ...
                if (commentData.email === email && commentData.rating !== null && commentData.id !== editId) {
    ...
    const formatTimestamp = (timestamp: string) => {
        ...
        return `${date.toLocaleDateString("de-DE", dateOptions)}, ${date.toLocaleTimeString("de-DE", timeOptions)} Uhr`;
    };

    const isEmailTaken = () => {
        return comments.some(c => c.email === email && c.rating !== null && c.id !== editId);
    };
            ...
            <div style={{ display: "grid", gridTemplateColumns: "max-content 1fr", gap: "10px 20px", alignItems: "center" }}>
                ...
                {isLoggedIn ? (
                    ...
                    <>
                        ...
                        />
                        {emailError && <><div /><div className="missing">{emailError}</div></>}
                    </>
                )}
                ...
                {ratingError && <><div /><div className="missing">{ratingError}</div></>}
                {hasGivenRating && <><div /><div className="info">You have already given a rating for this recipe.</div></>}
                    ...
                    <div style={{ gridColumn: "1 / span 2" }}>
                        ...
                        <div>
                            {comments.map((c, index) => (
                                <div key={index} style={{ marginBottom: 10, borderTop: "1px solid grey", padding: 10 }}>
                                    {isLoggedIn && isAdmin && (
                                        <>
                                            <button onClick={() => handleEdit(c)} className="btn" style={{ float: "right" }}>Edit</button>
                                            <button onClick={() => handleDelete(c.id!)} className="btn" style={{ float: "right" }}>Delete</button>
                                        </>
                                    )}
                                    <div style={{ fontWeight: 600 }}>{c.name} <span style={{ fontWeight: 100 }}>({c.email})</span></div>
                                    <small>am {c.timestamp ? formatTimestamp(c.timestamp) : 'No timestamp'}</small>
                                    {c.rating && <div>Rating: {c.rating}</div>}
                                    {c.comment && <div style={{ paddingLeft: 10, borderLeft: "3px double rgba(0,0,0,0.2)", marginTop: 10 }}>{c.comment}</div>}
                                    <br /><br />
                                </div>
                            ))}
                        </div>
